...
class ID_GA_Layer(nn.Module):
    ''' Compose with two layers '''
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(ID_GA_Layer, self).__init__()
        self.GA_layer = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)

    def forward(self, visual_input, textual_input, non_pad_mask=None, slf_attn_mask=None):
        visual_output, visual_slf_attn = self.GA_layer(visual_input, textual_input, textual_input,
            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)
        textual_output = textual_input

        return visual_output, textual_output, visual_slf_attn

class SA_GA_Layer(nn.Module):
    ''' Compose with two layers '''
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(SA_GA_Layer, self).__init__()
        self.SA_layer = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
        self.GA_layer = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)

    def forward(self, visual_input, textual_input, non_pad_mask=None, slf_attn_mask=None):
        textual_output, _ = self.SA_layer(textual_input, textual_input, textual_input,
            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)
        visual_output, visual_slf_attn = self.GA_layer(visual_input, textual_output, textual_output,
                                non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)

        return visual_output, textual_output, visual_slf_attn

class SA_SGA_Layer(nn.Module):
    ''' Compose with two layers '''
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(SA_SGA_Layer, self).__init__()
        self.SA_layer_t = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
        self.SA_layer_v = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
        self.GA_layer = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)

    def forward(self, visual_input, textual_input, non_pad_mask=None, slf_attn_mask=None):
        textual_output, _ = self.SA_layer_t(textual_input, textual_input, textual_input,
                            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)
        visual_output, _ = self.SA_layer_v(visual_input, visual_input, visual_input,
                            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)
        enc_output, enc_slf_attn = self.GA_layer(visual_output, textual_output, textual_output,
                            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)

        return enc_output, textual_output, enc_slf_attn

class ID_SGA_Layer(nn.Module):
    ''' Compose with two layers '''
    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(ID_SGA_Layer, self).__init__()
        self.SA_layer_v = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
        self.GA_layer = Single_Att_Layer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout)
    ...
    def forward(self, visual_input, textual_input, non_pad_mask=None, slf_attn_mask=None):
        visual_output, _ = self.SA_layer_v(visual_input, visual_input, visual_input,
                            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)
        enc_output, enc_slf_attn = self.GA_layer(visual_output, textual_input, textual_input,
                            non_pad_mask=non_pad_mask, slf_attn_mask=slf_attn_mask)
