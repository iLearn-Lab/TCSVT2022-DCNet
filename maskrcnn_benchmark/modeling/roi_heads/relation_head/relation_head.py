...
from controling.control_module import suppose_first_classify_is_right
...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        """
        if self.training or suppose_first_classify_is_right:
        ...
        if self.training and relation_logits == None:
            output_losses = {}
            output_losses.update(add_losses)
            return roi_features, proposals, output_losses

        ''' 2 CE loss, one for obj and one for rel'''
