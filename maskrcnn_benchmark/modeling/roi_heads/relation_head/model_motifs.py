...
from controling.control_module import myhier
...
class FrequencyBias(nn.Module):
    ...
    def __init__(self, cfg, statistics, eps=1e-3):
        ...
        ''' xianjing '''
        self.obj_baseline_pattern = torch.zeros([self.num_objs * self.num_objs, len(myhier)+1])
        self.obj_baseline_pattern[:, 0] = pred_dist[:, 0]
        for i in range(len(myhier)):
            for j in myhier[i]:
                self.obj_baseline_pattern[:, i+1] = self.obj_baseline_pattern[:, i+1] + pred_dist[:, j]
            self.obj_baseline_pattern[:, i+1] = self.obj_baseline_pattern[:, i+1] / len(myhier[i])
        self.obj_baseline_pattern_emb = nn.Embedding(self.num_objs * self.num_objs, len(myhier) + 1)
        with torch.no_grad():
            self.obj_baseline_pattern_emb.weight.copy_(self.obj_baseline_pattern, non_blocking=True)
        ''' xianjing '''
    ...
    def index_with_labels_pattern(self, labels):
        ''' xianjing '''
        """
        :param labels: [batch_size, 2]
        :return:
        """
        return self.obj_baseline_pattern_emb(labels[:, 0] * self.num_objs + labels[:, 1])
