...
class SACell(nn.Module):
    def __init__(self, config, hidden_dim=None):
        super(SACell, self).__init__()
        self.cfg = config
...
class Single_Layer_SCA(nn.Module):
    """
    A encoder model with self attention mechanism.
    """
    def __init__(self, config):
        super().__init__()
        self.SA_Cell_vis = SACell(config)
        self.SA_Cell_txt = SACell(config)
        self.CA_Cell_vis = CACell(config)
        self.CA_Cell_txt = CACell(config)
...
class Context_SCA(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.context_obj = Encoder_SCA(config, self.obj_layer)
        self.context_edge = Encoder_SCA(config, self.edge_layer)
