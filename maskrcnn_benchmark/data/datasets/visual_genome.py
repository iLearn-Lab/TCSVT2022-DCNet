...
from controling.control_module import use_gt_box
...
class VGDataset(torch.utils.data.Dataset):
    ...
    def relabel_the_data_by_pre_trained_model(self):
        from controling.Absolute_Path import relabel_file_dir_train, relabel_file_dir_test, relabel_file_dir_val
        if use_gt_box:
            suffix = ''
        else:
            suffix = '_sgdet'
        if self.split == 'train':
            relabel_file_dir = relabel_file_dir_train
        elif self.split == 'val':
            relabel_file_dir = relabel_file_dir_val
        elif self.split == 'test':
            relabel_file_dir = relabel_file_dir_test
        relabel_file_dir = relabel_file_dir[:-5]+suffix+'.json'
        print('\nthe preload file we use is %s' % relabel_file_dir)

        with open(relabel_file_dir, encoding='gbk') as file_obj:
            loaded_data_all = json.load(file_obj)
        pre_labels = []
        for i in range(len(loaded_data_all)):
            assert loaded_data_all[i]['img_file'] == os.path.basename(self.filenames[i])
            assert loaded_data_all[i]['rel_obj'] == len(self.gt_classes[i])
            pre_rel_matrix = loaded_data_all[i]['pred_matrix']
            pre_labels.append(pre_rel_matrix)

        print('\nyeah:'+self.split)
        return pre_labels

    def generate_full_connect_graph(self):
        edge_message = []
        for i in range(len(self.gt_boxes)):
            num_obj = len(self.gt_classes[i])
            edge_2o = []
            for m in range(num_obj):
                for n in range(num_obj):
                    if m != n:
                        pair = [m, n]
                        edge_2o.append(pair)
            edge_2o = np.array(edge_2o, dtype=int)
            edge_o2 = edge_2o.T
            edge_message.append(edge_o2)
        return edge_message
